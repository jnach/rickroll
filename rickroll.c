...
static int __init rickroll_init(void) {
	...
	if (major < 0) {
		printk(KERN_ALERT "Rick has failed us\n");
		return major;
	}

	printk(KERN_INFO "Rickroll ahoy %i\n", major);
	return 0;
}

static void __exit rickroll_exit(void) {
	unregister_chrdev(major, DEVICE_NAME);
	printk(KERN_INFO "Rickroll mode deactivated\n");
}
...
static ssize_t dev_write(struct file *filep, const char *buffer,
	...
	printk(KERN_INFO "Sorry, Rick doesn't play nicely with writes\n");
	return -EFAULT;
}

static int dev_release(struct inode *inodep, struct file *filep) {
	printk(KERN_INFO "Rick has gone away\n");
	return 0;
}
